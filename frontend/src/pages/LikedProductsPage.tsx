...
const LikedProductsPage: React.FC = () => {
    ...
    const load = async (reset = false) => {
        ...
        setLoading(true);
        try {
            const res = await API.get(`/user/liked-products?limit=12${reset || !cursor ? '' : `&cursor=${cursor}`}`, token);
            const data = res.data;
    ...
    useEffect(() => { load(true); }, [token]);
        // Auto refresh when a like toggle happens anywhere in the app
        useEffect(() => {
            if (!token) return;
            if (!logs.length) return;
            const last = logs[logs.length - 1];
            if (last.t === lastProcessedAction.current) return;
            if (last.type === 'like_toggle') {
                lastProcessedAction.current = last.t;
                // Re-fetch first page to reflect additions/removals
                setCursor(null);
                load(true);
            }
        }, [logs, token]);
